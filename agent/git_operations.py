...
class GitOperations:
    ...
    def create_fix_branch(self, fix_id: str, attempt: int = 1,
                         base_branch: str = "main") -> BranchInfo:
        """
        Create autonomous fix branch

        NOTE: Branch name does NOT include attempt number.
        Attempts are tracked via commit messages and labels.

        Args:
            fix_id: Unique fix identifier
            attempt: Attempt number (for metadata only)
            base_branch: Base branch to branch from

        Returns:
            BranchInfo with branch details
        """
        branch_name = self.config.format_branch_name(fix_id)
...
"""
    ...
    def get_commits_on_branch(self, branch_name: str, exclude_base: str = 'main') -> List:
        """
        Get commits unique to specified branch (not in base branch)

        Args:
            branch_name: Branch name to get commits from
            exclude_base: Base branch to exclude commits from (default: 'main')

        Returns:
            List of commit objects (GitPython format) unique to this branch
        """
        logger.info(f"Getting commits on branch: {branch_name}")
        ...
        if self.mock_mode:
            ...
            return [
                MockCommit(
                    message="🤖 Autonomous Fix Attempt 1: Mock fix",
                    hexsha="mock_sha_123",
                    author=MockAuthor(name="Autonomous Agent", email="agent@apra.ai")
                )
            ]

        try:
            # Get commits unique to this branch (not in base branch)
            # Use git log origin/base..branch syntax to get only new commits
            try:
                commits = list(self.git_repo.iter_commits(f'origin/{exclude_base}..{branch_name}'))
            except:
                # Fallback to local base branch if origin not available
                commits = list(self.git_repo.iter_commits(f'{exclude_base}..{branch_name}'))

            logger.info(f"Found {len(commits)} commits on {branch_name}")
            return commits
        except Exception as e:
            logger.warning(f"Could not get commits for {branch_name}: {e}")
            return []
    ...
    def get_diff(self, branch: str, base: str = 'main') -> str:
        ...
        Returns:
            Diff string
        """
        logger.info(f"Getting diff: {base}...{branch}")
...
"""
    ...
    def find_issue_by_label(self, label: str):
        ...
        Returns:
            GitHub Issue object or None
        """
        logger.info(f"Searching for issue with label: {label}")
        ...
        try:
            ...
            for issue in issues:
                logger.info(f"Found existing issue: {issue.html_url}")
    ...
    def create_pull_request(self,
                           ...
                           branch: str,
                           base: str = 'main',
                           labels: Optional[List[str]] = None) -> Optional[PRInfo]:
        """
        Create pull request (new method for CASE 3)
        ...
        Returns:
            PRInfo if successful
        """
        logger.info(f"Creating PR: {title}")

        if self.mock_mode or not self.github_repo:
            logger.info(f"Mock PR created: {branch} -> {base}")
            return PRInfo(
                number=999,
                url=f"https://github.com/mock/mock/pull/999",
                title=title,
                branch=branch
            )
        ...
        try:
            ...
            existing_prs = self.github_repo.get_pulls(
                state='open',
                head=f"{self.github_repo.owner.login}:{branch}"
            )
    ...
    def create_issue(self,
        ...
        """
        Create GitHub issue (for CASE 5 escalation)
        ...
        Returns:
            GitHub Issue object or None
        """
        logger.warning(f"Creating escalation issue: {title}")

        if self.mock_mode or not self.github_repo:
            logger.info("Mock issue created")
            @dataclass
            class MockIssue:
                number: int = 888
                html_url: str = "https://github.com/mock/mock/issues/888"
            return MockIssue()
        ...
        try:
            ...
            logger.warning(f"Escalation issue created: {issue.html_url}")
            return issue
